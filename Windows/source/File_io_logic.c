...
NTSTATUS ALL_in_ONE_FILE_IO(PVOID* RAW_DATA, ULONG* RAW_DATA_SIZE, UNICODE_STRING File_path, ALL_in_ONE_FILE_IO_info custom_MODE) {
    ...
    case    READ_MODE_with_FILE_STANDARD_INFORMATION__get__FILE_LENGTH: // 파일 길이 얻을 때
       ...
       // }
        ...
        status = Create_FILE(&filehandle, File_path, &objAttr, &ioStatusBlock, FILE_GENERIC_READ | SYNCHRONIZE, FILE_SHARE_READ , FILE_OPEN);
        if (status != STATUS_SUCCESS) {
            //  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "Create_FILE - READ -> Create_FILE 결과 %p ", status);
            return status;
        }
...
BOOLEAN FILE_to_INFO(
    ...
    ULONG File_Size = 0;
    NTSTATUS s = ALL_in_ONE_FILE_IO(&File_Buffer, &File_Size, *file_path, READ_MODE_with_FILE_STANDARD_INFORMATION__get__FILE_LENGTH);
    if ( s!= STATUS_SUCCESS) {
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[ALL_in_ONE_FILE_IO] 실패: %p / %wZ \n", s, file_path);
        return_bool = FALSE;
        if (opt_pid != NULL) {
            goto EXIT2;
        }
