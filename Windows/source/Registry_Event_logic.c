...
NTSTATUS ExRegistryCallback_for_monitor(PVOID CallbackContext, PVOID Argument1, PVOID Argument2) {
    ...
    //DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[레지스트리] - 부모 정보 PPID: %llu // ImageName: %wZ\n", PPID, Parent_ImageName);
        ...
        */
        ...
        if (Status == STATUS_SUCCESS)
        {
            ...
            /*
                3. 설정 값 이름
            */
            ValueName = pSetValueInfo->ValueName;
            UNICODE_to_ANSI(&ValueNameA, ValueName);
            current = AppendDynamicData(current, (PUCHAR)ValueNameA.Buffer, ValueNameA.MaximumLength - 1);
            UNICODE_to_ANSI_release(&ValueNameA);

            /*
                4. (3) 이름의 타입
            */
            ULONG Type = pSetValueInfo->Type;
            current = AppendDynamicData(current, (PUCHAR)&Type, sizeof(Type));

            /*
                5. 실제 데이터 (길이가 0인경우 주의)
            */
            if (pSetValueInfo->DataSize > 0) {
                current = AppendDynamicData(current, (PUCHAR)pSetValueInfo->Data, pSetValueInfo->DataSize);
            }
    ...
    case RegNtPreDeleteValueKey:
    {
       ...
       */
        ...
        if (Status == STATUS_SUCCESS)
        {
            ...
            // 삭제하려는 값이름
            ValueName = pDeleteValueInfo->ValueName;
            UNICODE_to_ANSI(&ValueNameA, ValueName);
            current = AppendDynamicData(current, (PUCHAR)ValueNameA.Buffer, ValueNameA.MaximumLength - 1);
            UNICODE_to_ANSI_release(&ValueNameA);
    ...
    case RegNtPreRenameKey:
    {
       ...
       */
        ...
        if (Status == STATUS_SUCCESS)
        {
            ...
            // New Value name .추출
            ValueName = pRenameKeyInfo->NewName;
            UNICODE_to_ANSI(&ValueNameA, ValueName);
            current = AppendDynamicData(current, (PUCHAR)ValueNameA.Buffer, ValueNameA.MaximumLength - 1);
            UNICODE_to_ANSI_release(&ValueNameA);
