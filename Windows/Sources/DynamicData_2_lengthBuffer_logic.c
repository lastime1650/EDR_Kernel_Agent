...
VOID Dyn_2_lenBuff(Pmover in_parameter) {
	...
	case PsSetCreateProcessNotifyRoutine_Creation_Detail:
	{
		...
		PCHAR EXE_NAME = (PCHAR)current->Data;														// 4. (EXE_FILE_NAME)
		SIZE += append_size(current->Size);

		SIZE += append_Node_pid_path(current, &current, in_parameter->PID);							// 무조건 마지막에 처리해야함 . ( SHA256 + FILE_SIZE ) 
	...
	case PsSetLoadImageNotifyRoutine_Load:
	{
		...
		/*꺼내기*/
		PWCH ImagePath_W = (PWCH)current->Data;														// 1.
		ANSI_STRING ImagePath_A = { 0, };
		PWCH_to_ANSI(&ImagePath_A, ImagePath_W);
		Change_Node_Data(current, (PUCHAR)ImagePath_A.Buffer, ImagePath_A.MaximumLength - 1);
		PWCH_to_ANSI_release(&ImagePath_A);
		...
		// Image 에 대한 SHA256 구하기 
		UNICODE_STRING ImagePATH_unicode = { 0, };
		RtlInitUnicodeString(&ImagePATH_unicode, ImagePath_W);
		ULONG32 image_size = 0;
		...
		if (!FILE_to_INFO(
			...
			current = AppendDynamicData(current, (PUCHAR)SHA256, (SHA256_String_Byte_Length - 1)); // 4. 이미지에 대한 SHA256
	...
	case File_System:
	{
		...
		*/
		...
		if (memcmp(File_Behavior, "Rename", sizeof("Rename") - 1) == 0) {
			current = (PDynamicData)current->Next_Addr;
			if (current) {
				PWCH Rename_File_Path_W = (PWCH)current->Data;												// 4.
	...
	case NDIS_Network_Traffic:
	{
		...
		*/
		...
		PUCHAR LOCAL_IPv4_STR = current->Data;
		SIZE += append_size(current->Size);
		current = (PDynamicData)current->Next_Addr;

		ULONG32 LOCAL_PORT = *(ULONG32*)current->Data;
		SIZE += append_size(current->Size);
		current = (PDynamicData)current->Next_Addr;
		*/
...

	{
		...
		if (SEND_DATA && SEND_DATA_SIZE > 99999) {
			ExFreePoolWithTag(SEND_DATA, LEN_BUFF_TAG);
			SEND_DATA = NULL;
			SEND_DATA_SIZE = 0;
		}
